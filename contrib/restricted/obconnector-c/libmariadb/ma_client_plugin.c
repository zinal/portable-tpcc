                    ...
                    int argc, va_list args)
{
  ...
  char *obclient_plugin_dir = getenv("OBCLIENT_PLUGIN_DIR");
  ...
  snprintf(dlpath, sizeof(dlpath) - 1, "%s/%s%s",
           ...
           mysql->options.extension->plugin_dir : (obclient_plugin_dir) ?
           obclient_plugin_dir : (env_plugin_dir) ?
           env_plugin_dir : MARIADB_PLUGINDIR, name, SO_EXT);
#else
  {
