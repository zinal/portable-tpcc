...
TDebugReport RunObDebugProbe(
    const std::string& connection,
    const std::string& path,
    TDebugProbeRequest req)
{
    TObConnectionPool pool(connection, 1, 1, path);
    TObSessionFactory factory(pool);
    ...
    int repeats)
{
    ...
    return RunOrchestratedDebug(doc, instance, repeats,
        [](const TRunConfigDocument& d, TDebugProbeRequest req) {
            const std::string connection = BuildObConnectionString(d);
            CheckDbForRun(connection, d.ScaleWarehouses, d.Path);
            return RunObDebugProbe(connection, d.Path, req);
        });
}
    ...
    int repeats)
{
    ...
    req.DistrictID = 1;
    req.Repeats = repeats;
    auto report = RunObDebugProbe(connectionString, path, req);
