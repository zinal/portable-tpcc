...
| `threads_per_worker` | `0` | Worker coroutine threads. `0` / omit keeps `threads: 0` in the assignment so each worker applies the same CPU + warehouse auto as standalone `--threads=0` / tpcc-postgres-cpp (see `ComputeRunLayout`, ≈ `ceil(warehouses / 1000)`). Explicit `N > 0` pins that many threads per worker. Auto sizing is useful when `ITpccTransaction` does not block the scheduler (PostgreSQL worker path). Until OceanBase / YDB async adapters land ([async-adapter-transactions.md](async-adapter-transactions.md)), pin `threads_per_worker` near the desired concurrency for those DBMS; otherwise `Inflight` stays ≈ `ThreadCount`. |
