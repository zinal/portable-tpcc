...
| `query_timeout=` in `--connection` | `database.options.query_timeout` | Session `ob_query_timeout` for bulk import / `CREATE INDEX` / `DBMS_STATS` / integrity checks / pre-flight catalog queries, in **seconds** (default **600**). OceanBase server default is 10s. |
...
`Checking [i/n] … [OK]` appears as each check finishes (shared
`RecordCheckResult` helper; specification §9.2). Under `mind-tpcc`,
...
Worker `ITpccTransaction` does not block the scheduler on MariaDB/OceanBase
connector IO. Paced runs can keep `--threads` / `threads_per_worker` low
(including auto `threads=0`) and a large `--max_inflight` /
...
database:
  ...
  options:
    ...
    query_timeout: 600          # bulk import / CREATE INDEX / DBMS_STATS / check / pre-flight (seconds)
