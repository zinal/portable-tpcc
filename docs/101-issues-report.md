...
Результат после прохода исправлений для IR-001..IR-020:

- **устранено:** 20;
- **частично устранено:** 0;
- **остаётся:** 0.

IR-003 и IR-018 были устранены ранее. На ветке
`cursor/fix-101-issues-e7cf` закрыты оставшиеся замечания, включая IR-019 и
IR-020: добавлены fail-fast проверки, безопасная обработка артефактов,
устойчивый lifecycle orchestration и приведение настроек run-config к
...
до формирования SSH-команды. Дополнительно профиль с некорректным SSH key
отклоняется на этапе validation.
...
После исправлений на ветке `cursor/fix-101-issues-e7cf` проверки были
перезапущены:
...
Оставшийся риск тестирования: отчёт фиксирует успешный regression run
локальных Go и ya tests; расширенный интеграционный прогон с внешним
PostgreSQL в нём не приводится.

## 7. Выполненный порядок исправления

1. IR-001 и IR-002 — закрыты границы удалённого выполнения и сбора данных.
2. IR-004—IR-006 и IR-016 — исправлены lifecycle, lock и идентичность
   артефактов.
3. IR-007—IR-012 и IR-017 — добавлены fail-fast validation и гарантии
   adapter/runtime.
4. IR-013—IR-015 и IR-019—IR-020 — настройки согласованы с поведением и
   расширены проверки.

После этого повторно выполнены команды проверки из раздела 6:
`go -C tpccctl test ./...`, `go -C tpccctl build ./cmd/tpccctl` и
`./ya make -t tpcc`.
