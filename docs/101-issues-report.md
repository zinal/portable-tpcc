...
Результат контрольной проверки для IR-001..IR-020 с учётом остаточных
исправлений:
...
run-config. Остаточный проход на ветке `cursor/fix-residual-101-e7cf` закрыл
оставшиеся граничные случаи IR-001, IR-004, IR-006, IR-009, IR-010 и IR-017.
...
Все IR-001..IR-020 закрыты. Остаточные граничные случаи IR-001, IR-004,
IR-006, IR-009, IR-010 и IR-017 устранены на ветке
`cursor/fix-residual-101-e7cf`.

Итоговое состояние:

1. **устранено:** 20;
2. **частично устранено:** 0;
3. **остаётся:** 0.

Дальнейшее сопровождение сводится к сохранению regression coverage для
границ артефактов, state pipeline, process nonce, DML cardinality,
connection-pool shutdown и warehouse assignment invariants.
