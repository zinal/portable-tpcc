...
Контрольная проверка выполнена на commit `855680d` (`main`). Основные сценарии
IR-001..IR-020 исправлены, однако для шести замечаний обнаружены остаточные
граничные случаи. Поэтому их статус уточнён до «частично устранено».
...
Результат контрольной проверки для IR-001..IR-020:

- **устранено:** 14;
- **частично устранено:** 6;
- **остаётся:** 0.
...
run-config. Контрольный аудит подтвердил эти исправления в основных сценариях,
но выявил остаточные риски в IR-001, IR-004, IR-006, IR-009, IR-010 и IR-017.
...
- **Устранено** — исправление и соответствующая проверка присутствуют в
  коде ветки `cursor/fix-101-issues-e7cf`.
...
**Остаточный риск:** `JoinUnder()` выполняет лексическую проверку пути, но не
проверяет symlink-компоненты на runtime host. Скомпрометированный worker может
создать symlink внутри instance directory на файл вне этого каталога и указать
лексически безопасный payload path. Для полной защиты требуется allowlist
известных payload-файлов либо проверка разрешённого remote path без следования
symlink.
...
**Остаточный риск:** stage-команды вызывают `Materialize()` до захвата profile
lock (`tpccctl/internal/cli/cli.go:164-188`). `Materialize()` выполняет
load/save `run-state.json`, поэтому параллельная команда способна перезаписать
более новое состояние или process metadata активного orchestrator ещё до
отказа на lock. Кроме того, `Transition()` не проверяет допустимость перехода
между фазами.
...
**Остаточный риск:** `launchRole()` игнорирует ошибку/таймаут получения
`process.json`. Если `InstanceNonce` остаётся пустым, `waitProcesses()` не
выполняет nonce comparison и может принять старый finalized manifest
(`tpccctl/internal/orchestrator/drive.go:178,234-277`). Получение metadata
должно быть обязательным условием успешного запуска процесса.
...
**Остаточный риск:** Delivery проверяет только, что обновлена хотя бы одна
`order_line`. Частичное обновление, например 3 строки из ожидаемых 10,
считается успешным (`tpcc/dbms/pgsql/tpcc_session.cpp:564-571`). Ожидаемое
число строк известно из `TGetDeliveryOrderInfo::LineCount` и должно
сравниваться с `affected_rows`.
...
**Остаточный риск:** если `CreateConnection()` при замене завершается ошибкой,
слот удаляется из pool без восстановления
(`tpcc/dbms/pgsql/pg_connection_pool.cpp:79-88`). Повторные ошибки способны
уменьшить pool до нуля; ожидающие `AcquireSession()` не получают условие
завершения и могут зависнуть. Необходима fail-fast остановка pool либо
сохранение/асинхронное восстановление его ёмкости.
...
**Остаточный риск:** проверяются отдельные warehouse ranges, но не их
пересечения, дублирование instance names и полнота покрытия scale. Ручной или
повреждённый run-config с двумя worker на одном warehouse проходит C++
validation и искажает workload. Consumer должен повторять assignment
validation независимо от доверия к Go producer.
...
На commit `855680d` контрольные проверки перезапущены:
...
Оставшиеся пробелы тестирования:

- нет PostgreSQL integration test для ошибки пересоздания соединения и
  сохранения ёмкости pool;
- нет adapter test, который моделирует частичное обновление `order_line`;
- нет C++ unit tests пересекающихся/дублирующихся warehouse assignments;
- нет теста запуска с отсутствующим или запаздывающим `process.json`;
...
Основные сценарии всех IR-001..IR-020 закрыты. До полного закрытия контрольного
аудита остаются шесть граничных случаев, перечисленных в IR-001, IR-004,
IR-006, IR-009, IR-010 и IR-017. Приоритет:

1. не допускать приём manifest без подтверждённого nonce и исключить symlink
   escape;
2. не допускать уменьшение connection pool до состояния вечного ожидания;
3. проверять полную DML cardinality Delivery и assignment invariants;
4. переносить materialization под profile lock и проверять state transitions;
5. добавить integration/regression tests из раздела 6.
