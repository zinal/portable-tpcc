...
func TestRequireCompletedLoad(t *testing.T) {
	cases := []struct {
		name    string
		state   string
		skipped []string
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			err := requireCompletedLoad(rs, "after-import")
			if tc.wantErr {
...
func TestRequireCheckPhase(t *testing.T) {
	cases := []struct {
		...
		{name: "after_import_needs_load", phase: "after-import", state: state.StateLoading, wantErr: "requires a completed load"},
		...
		{name: "after_test_needs_load", phase: "after-test", state: state.StateLoading, wantErr: "requires a completed load"},
		{name: "after_test_skipped_indexes", phase: "after-test", state: state.StateLoading, skipped: []string{"indexes"}},
